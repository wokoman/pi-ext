...
import type { ChangedFile, RenderOptions } from "./types.js";
 ...
 *
 * The worker must receive an initTheme message before it becomes ready.
 */
export class SSRWorker {
  ...
  private themeInit: ThemeInit | null = null;

  /** Set theme data before first ensure(). */
  setTheme(theme: ThemeInit) {
  ...
  async ensure(): Promise<void> {
    ...
    this.starting = new Promise<void>((resolve, reject) => {
      ...
      // Send init theme immediately after spawn
      if (this.themeInit) {
        try {
          this.proc.stdin!.write(
            JSON.stringify({ id: 0, initTheme: this.themeInit }) + "\n",
          );
        } catch {}
      }
