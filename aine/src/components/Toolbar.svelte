<script lang="ts">
  ...
  interface Props {
    ...
    onToggleExpand: () => void;
    onDiffTypeChange: (type: string) => void;
    currentDiffType: string;
  }

  let { onRefresh, onToggleStyle, onToggleWhitespace, onToggleExpand, onDiffTypeChange, currentDiffType }: Props = $props();
</script>
...
<div id="toolbar">
  ...
  <!-- Section 1b: Diff type switcher -->
  <div class="t-section t-diff-type">
    <select
      class="diff-type-select"
      value={currentDiffType}
      onchange={(e) => onDiffTypeChange((e.target as HTMLSelectElement).value)}
    >
  ...
  <button
    class="toggle"
    class:on={$commentsPanelVisible}
    title="Toggle inline comments (c)"
